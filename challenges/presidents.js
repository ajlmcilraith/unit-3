...
let living = presidentsArray.filter((president) => president.passed === undefined)

let deadPresidents = presidentsArray.filter((president) => president.passed !== undefined)
...
deadPresidents.forEach((president) => {
  lifeTime = president.passed - president.year;
  //let { passed, year, first, last } = president
  // let age = passed - year
  // so passed === president.passed
  console.log(
    `${president.first} ${president.last} was ${lifeTime} years old when they died.`
  )
})

// ! Destructuring an Object
// * This will be helpful once we get to react
let person1 = {
  firstName: "Billy",
  lastName: "Idol"
}
...
let {firstName: first, lastName} = person1
